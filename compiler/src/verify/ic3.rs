            ...
            for slot in 0..n_slots {
                ...
                for (fi, f) in entity.fields.iter().enumerate() {
                    if let Some(ref constraint) = f.initial_constraint {
                        ...
                        {
                            create_guard =
                                format!("(and {create_guard} {constraint_smt})");
                        }
                    }
                }
...
fn constraint_to_smt_with_dollar(
    ...
    match expr {
        ...
        IRExpr::BinOp {
            ...
            let l = constraint_to_smt_with_dollar(left, dollar_var, entity, vctx)?;
            let r = constraint_to_smt_with_dollar(right, dollar_var, entity, vctx)?;
            match op.as_str() {
                "OpEq" => Ok(format!("(= {l} {r})")),
        ...
        IRExpr::UnOp { op, operand, .. } if op == "OpNot" => {
            let inner = constraint_to_smt_with_dollar(operand, dollar_var, entity, vctx)?;
            Ok(format!("(not {inner})"))
        }
        _ => Err(format!(
...
pub fn try_ic3_liveness(
    ...
    if let Err(e) = fp.from_string(&chc) {
        return Ic3Result::Unknown(format!("liveness CHC parse error: {e}"));
    }
...
fn build_liveness_chc(
    ...
    let trigger_smt = if let (Some(_var), Some(ent_name)) =
        (entity_var, entity_name_for_binding)
    {
        let entity = entities
            .iter()
            .find(|e| e.name == ent_name)
        ...
        if let Some(slot) = target_slot {
            ...
            for slot in 0..n_slots {
                ...
                let p = guard_to_smt_sys(trigger, entity, vctx, ent_name, slot)?;
                let q = guard_to_smt_sys(response, entity, vctx, ent_name, slot)?;
                slot_triggers.push(format!(
                    "(and {active} {p} (not {q}) (not mon_pending))"
                ));
            }
    ...
    let response_smt = if let (Some(_var), Some(ent_name)) =
        (entity_var, entity_name_for_binding)
    {
        let entity = entities
            .iter()
    ...
    let discharged_next_str = format!(
        "(ite (and mon_pending {response_smt}) true mon_discharged)"
    );
            ...
            for slot in 0..n_slots {
                for transition in &entity.transitions {
                    let guard =
                        guard_to_smt_sys(&transition.guard, entity, vctx, &entity.name, slot)?;
                    let entity_next_str =
                        build_transition_next(entities, slots_per_entity, entity, &entity.name, slot, transition, vctx)?;
                    let active_var = format!("{}_{}_active", entity.name, slot);
                ...
                // Create rule
                let create_next_str =
                    build_create_next(entities, slots_per_entity, entity, &entity.name, slot, &[], vctx)?;
                let inactive_var = format!("{}_{}_active", entity.name, slot);
                let mut create_guard = if slot == 0 {
                    format!("(not {inactive_var})")
                } else {
                    ...
                    for (oi, op) in ops.iter().enumerate() {
                        ...
                        } = op
                        {
                            let trans = entity
                                ...
                                .ok_or_else(|| {
                                    format!(
                                        "transition {trans_name} not found on entity {ent_name}"
                                    )
                                })?;
                            let trans_guard =
                        ...
                        } = op
                        {
                                ...
                                entity.transitions.iter().find(|t| t.name == *trans_name)
                            {
                                let trans_guard = guard_to_smt_sys(
                                    &trans.guard, entity, vctx, ent_name, slot,
                                )?;
                                let entity_next_str = build_transition_next(
                                    entities, slots_per_entity, entity, ent_name, slot, trans, vctx,
                                )?;
                                let mut all_guards = nested_guards.clone();
                                all_guards.push(trans_guard);
