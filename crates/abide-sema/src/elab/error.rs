...
impl ElabError {
    ...
    pub fn to_diagnostic(&self) -> Diagnostic {
        let mut diagnostic = Diagnostic {
            ...
            code: Some(format!("abide::{}", self.kind.code())),
            ...
            help: self
                .help
                .clone()
                .or_else(|| Some(self.kind.explanation().to_owned())),
            related: Vec::new(),
        };

        if let Some(secondary_span) = self.secondary_span {
            diagnostic.related.push(RelatedDiagnostic {
                ...
                file: self.secondary_file.clone().or_else(|| self.file.clone()),
            });
        }
...
mod tests {
    ...
    fn cross_file_secondary_produces_one_label() {
        ...
        .in_file("a.ab".to_owned())
        .with_secondary_in_file(Span { start: 10, end: 20 }, "first declared here", "b.ab");
        let labels: Vec<_> = err.labels().unwrap().collect();
        assert_eq!(
            labels.len(),
            1,
    ...
    #[test]
    fn to_diagnostic_preserves_warning_severity_and_help() {
        let diagnostic = ElabError::warning("suspicious", "ctx")
            .with_help("custom help")
            .to_diagnostic();
        assert_eq!(diagnostic.severity, DiagnosticSeverity::Warning);
        assert_eq!(diagnostic.help.as_deref(), Some("custom help"));
        assert_eq!(diagnostic.code.as_deref(), Some("abide::E003"));
    }

    #[test]
    fn to_diagnostic_preserves_secondary_related_location() {
        let diagnostic = ElabError::with_span(
            ErrorKind::DuplicateDecl,
            "duplicate `X`",
            "duplicate here",
        ...
        .in_file("a.ab")
        .with_secondary_in_file(Span { start: 5, end: 10 }, "first declared here", "b.ab")
        .to_diagnostic();

        assert_eq!(diagnostic.related.len(), 1);
        assert_eq!(diagnostic.related[0].message, "first declared here");
        assert_eq!(diagnostic.related[0].span, Some(Span { start: 5, end: 10 }));
        assert_eq!(diagnostic.related[0].file.as_deref(), Some("b.ab"));
    }
}
