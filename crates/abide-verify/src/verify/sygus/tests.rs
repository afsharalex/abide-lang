...
fn bool_setcomp_cardinality() -> IRExpr {
    IRExpr::Card {
...
fn bool_setcomp_membership() -> IRExpr {
    IRExpr::Index {
...
fn bool_set_union_cardinality() -> IRExpr {
    let bool_set_ty = IRType::Set {
        element: Box::new(IRType::Bool),
    };
    IRExpr::Card {
...
fn bool_set_subset() -> IRExpr {
    let bool_set_ty = IRType::Set {
        element: Box::new(IRType::Bool),
    };
    IRExpr::BinOp {
        op: "OpSetSubset".to_owned(),
        left: Box::new(IRExpr::SetLit {
            elements: vec![bool_lit(false)],
            ty: bool_set_ty.clone(),
            span: None,
...
fn bool_set_disjoint() -> IRExpr {
    let bool_set_ty = IRType::Set {
        element: Box::new(IRType::Bool),
    };
    IRExpr::BinOp {
        op: "OpDisjoint".to_owned(),
        left: Box::new(IRExpr::SetLit {
            elements: vec![bool_lit(false)],
            ty: bool_set_ty.clone(),
            span: None,
...
fn bool_map_update_lookup() -> IRExpr {
    ...
    IRExpr::Index {
        map: Box::new(IRExpr::MapUpdate {
            map: Box::new(IRExpr::MapLit {
                entries: vec![(bool_lit(false), bool_lit(false))],
                ty: map_ty.clone(),
                span: None,
            }),
            key: Box::new(bool_lit(true)),
...
fn bool_map_update_cardinality() -> IRExpr {
    let map_ty = IRType::Map {
        key: Box::new(IRType::Bool),
        value: Box::new(IRType::Bool),
    };
    IRExpr::Card {
        expr: Box::new(IRExpr::MapUpdate {
            map: Box::new(IRExpr::MapLit {
                entries: vec![(bool_lit(false), bool_lit(false))],
                ty: map_ty.clone(),
                span: None,
...
#[test]
fn sygus_system_step_supports_finite_setcomp_cardinality_guards() {
    let tm = Cvc5Tm::new();
    let mut system = make_counter_system();
    system.actions[0].guard =
        bin_expr("OpEq", bool_setcomp_cardinality(), int_lit(1), IRType::Bool);
    let enum_catalog = build_enum_catalog(&tm, &system.fields).expect("enum catalog should build");
    let mut curr_vars = HashMap::new();
    let mut next_vars = HashMap::new();
    for field in &system.fields {
...
#[test]
fn sygus_system_step_supports_finite_setcomp_membership_guards() {
    let tm = Cvc5Tm::new();
    let mut system = make_counter_system();
    system.actions[0].guard = bool_setcomp_membership();
    let enum_catalog = build_enum_catalog(&tm, &system.fields).expect("enum catalog should build");
    let mut curr_vars = HashMap::new();
    let mut next_vars = HashMap::new();
    for field in &system.fields {
...
#[test]
fn sygus_system_step_supports_finite_set_union_cardinality_guards() {
    let tm = Cvc5Tm::new();
    let mut system = make_counter_system();
    system.actions[0].guard = bin_expr(
        "OpEq",
        bool_set_union_cardinality(),
        int_lit(2),
        IRType::Bool,
    );
    let enum_catalog = build_enum_catalog(&tm, &system.fields).expect("enum catalog should build");
    ...
    .unwrap_or_else(|err| panic!("finite set union cardinality guard should encode: {err}"));
}

#[test]
fn sygus_system_step_supports_finite_set_subset_guards() {
    let tm = Cvc5Tm::new();
    let mut system = make_counter_system();
    system.actions[0].guard = bool_set_subset();
    let enum_catalog = build_enum_catalog(&tm, &system.fields).expect("enum catalog should build");
    let mut curr_vars = HashMap::new();
    let mut next_vars = HashMap::new();
    for field in &system.fields {
    ...
    .unwrap_or_else(|err| panic!("finite set subset guard should encode: {err}"));
}

#[test]
fn sygus_system_step_supports_finite_set_disjoint_guards() {
    let tm = Cvc5Tm::new();
    let mut system = make_counter_system();
    system.actions[0].guard = bool_set_disjoint();
    let enum_catalog = build_enum_catalog(&tm, &system.fields).expect("enum catalog should build");
    let mut curr_vars = HashMap::new();
    let mut next_vars = HashMap::new();
    for field in &system.fields {
    ...
    .unwrap_or_else(|err| panic!("finite set disjoint guard should encode: {err}"));
}
...
#[test]
fn sygus_system_step_supports_finite_map_update_lookup_guards() {
    let tm = Cvc5Tm::new();
    let mut system = make_counter_system();
    system.actions[0].guard = bool_map_update_lookup();
    let enum_catalog = build_enum_catalog(&tm, &system.fields).expect("enum catalog should build");
    let mut curr_vars = HashMap::new();
    let mut next_vars = HashMap::new();
    for field in &system.fields {
    ...
    .unwrap_or_else(|err| panic!("finite map update lookup guard should encode: {err}"));
}

#[test]
fn sygus_system_step_supports_finite_map_update_cardinality_guards() {
    let tm = Cvc5Tm::new();
    let mut system = make_counter_system();
    system.actions[0].guard = bin_expr(
        "OpEq",
        bool_map_update_cardinality(),
        int_lit(2),
        IRType::Bool,
    );
    let enum_catalog = build_enum_catalog(&tm, &system.fields).expect("enum catalog should build");
    ...
    .unwrap_or_else(|err| panic!("finite map update cardinality guard should encode: {err}"));
}
...
#[test]
fn sygus_pooled_system_step_supports_finite_setcomp_cardinality_guards() {
    let tm = Cvc5Tm::new();
    let entity = make_pooled_counter_entity();
    let mut system = make_pooled_counter_system();
    system.actions[0].guard =
...
#[test]
fn sygus_pooled_system_step_supports_finite_setcomp_membership_guards() {
    let tm = Cvc5Tm::new();
    let entity = make_pooled_counter_entity();
    let mut system = make_pooled_counter_system();
    system.actions[0].guard = bool_setcomp_membership();
...
#[test]
fn sygus_pooled_system_step_supports_finite_set_union_cardinality_guards() {
    let tm = Cvc5Tm::new();
    let entity = make_pooled_counter_entity();
    let mut system = make_pooled_counter_system();
    system.actions[0].guard = bin_expr(
    ...
    .unwrap_or_else(|err| panic!("pooled finite set union cardinality guard should encode: {err}"));
}

#[test]
fn sygus_pooled_system_step_supports_finite_set_subset_guards() {
    let tm = Cvc5Tm::new();
    let entity = make_pooled_counter_entity();
    let mut system = make_pooled_counter_system();
    system.actions[0].guard = bool_set_subset();
    ...
    .unwrap_or_else(|err| panic!("pooled finite set subset guard should encode: {err}"));
}

#[test]
fn sygus_pooled_system_step_supports_finite_set_disjoint_guards() {
    let tm = Cvc5Tm::new();
    let entity = make_pooled_counter_entity();
    let mut system = make_pooled_counter_system();
    system.actions[0].guard = bool_set_disjoint();
    ...
    .unwrap_or_else(|err| panic!("pooled finite set disjoint guard should encode: {err}"));
}
...
#[test]
fn sygus_pooled_system_step_supports_finite_map_update_lookup_guards() {
    let tm = Cvc5Tm::new();
    let entity = make_pooled_counter_entity();
    let mut system = make_pooled_counter_system();
    system.actions[0].guard = bool_map_update_lookup();
    ...
    .unwrap_or_else(|err| panic!("pooled finite map update lookup guard should encode: {err}"));
}

#[test]
fn sygus_pooled_system_step_supports_finite_map_update_cardinality_guards() {
    let tm = Cvc5Tm::new();
    let entity = make_pooled_counter_entity();
    let mut system = make_pooled_counter_system();
    system.actions[0].guard = bin_expr(
