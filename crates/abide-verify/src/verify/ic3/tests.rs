...
#[test]
fn build_multi_slot_chc_supports_finite_payload_enum_quantifier_domains() {
    ...
    let chc = build_multi_slot_chc(&entity, &vctx, &property, 1)
        .expect("finite payload enum quantifier should encode in IC3 CHC");
...
#[test]
fn build_system_chc_supports_payload_field_projection_in_properties() {
    let result_ty = IRTypeEntry {
        name: "Result".to_owned(),
        ty: IRType::Enum {
            name: "Result".to_owned(),
    ...
    .expect("system CHC should encode payload field projection properties");
...
#[test]
fn build_multi_slot_chc_supports_payload_field_projection_in_transition_updates() {
    let result_ty = IRTypeEntry {
        name: "Result".to_owned(),
        ty: IRType::Enum {
            name: "Result".to_owned(),
    ...
    let ir = make_ir_for_entity(&order, vec![result_ty]);
    let vctx = VerifyContext::from_ir(&ir);
    let property = IRExpr::Always {
        body: Box::new(IRExpr::Forall {
            var: "o".to_owned(),
    ...
    let chc = build_multi_slot_chc(&order, &vctx, &property, 1)
        .expect("multi-slot CHC should encode payload field projections in transition updates");
    assert!(chc.contains("(code s0_f0)"));
}
...
fn multi_slot_value_encoder_supports_payload_field_projection() {
    ...
    let smt = expr_to_smt_scoped(&projection, &entity, &vctx, &HashSet::new()).expect(
        "multi-slot value encoder should project payload fields through datatype selectors",
    );

    assert_eq!(smt, "(code f0)");
}
...
fn multi_slot_choose_helpers_cover_finite_quantified_and_direct_witness_paths() {
    ...
    let finite_payload_ty = IRType::Enum {
        name: "Decision".to_owned(),
        variants: vec![
            IRVariant {
                name: "Accept".to_owned(),
    ...
    assert_eq!(
        ic3_finite_choose_candidates(&finite_payload_ty, &vctx),
        Some(vec![
            "(Accept false)".to_owned(),
            "(Accept true)".to_owned(),
            "Reject".to_owned()
        ])
    );
