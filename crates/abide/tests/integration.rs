...
#[test]
fn explicit_state_verifier_supports_payload_initial_field_constraints() {
    let src = r"module T
...
verify explicit_payload_initial_constraint {
  assume {
...
";
    ...
    assert!(
        results.iter().any(|result| matches!(
            result,
            abide::verify::VerificationResult::Proved { name, method, .. }
                if name == "explicit_payload_initial_constraint"
                    && method == "explicit-state exhaustive search"
        )),
        "payload initial field constraints should enumerate finite values through explicit-state, got: {results:?}"
    );
}

#[test]
fn explicit_state_verifier_supports_unconstrained_payload_initial_fields() {
    let src = r"module T
...
verify explicit_unconstrained_payload_initial_field {
  assume {
    store tickets: Ticket[1]
    let queue = Queue { tickets: tickets }
    stutter
  ...
  assert always all ticket: Ticket | ticket.decision.allowed or not ticket.decision.allowed
}
";
    ...
    assert!(
        results.iter().any(|result| matches!(
            result,
            abide::verify::VerificationResult::Proved { name, method, .. }
                if name == "explicit_unconstrained_payload_initial_field"
                    && method == "explicit-state exhaustive search"
        )),
        "unconstrained payload initial fields should enumerate finite values through explicit-state, got: {results:?}"
    );
}
...
#[test]
fn explicit_state_verifier_supports_entity_step_param_apply() {
    let src = r"module T
...
system Queue(tickets: Store<Ticket>) {
  command close_one(ticket: Ticket) {
    ticket.close()
  }
}

verify explicit_entity_step_param_apply {
  assume {
    store tickets: Ticket[1]
    let queue = Queue { tickets: tickets }
    stutter
...
";
    ...
    assert!(
        results.iter().any(|result| matches!(
            result,
            abide::verify::VerificationResult::Proved { name, method, .. }
                if name == "explicit_entity_step_param_apply"
                    && method == "explicit-state exhaustive search"
        )),
        "entity step parameters should be valid entity action targets in explicit-state, got: {results:?}"
    );
}
...
#[test]
fn explicit_state_verifier_supports_entity_choose_cross_call_returns() {
    let src = r"module T
...
system Picker(tickets: Store<Ticket>) {
  command pick_open() -> Ticket {
    return choose ticket: Ticket where ticket.status == @Open
  }
}

system Queue(tickets: Store<Ticket>) {
  command close_picked() {
    let ticket = Picker::pick_open()
    ticket.close()
  }
}

verify explicit_entity_choose_cross_call_return {
  assume {
    store tickets: Ticket[1]
    let picker = Picker { tickets: tickets }
    let queue = Queue { tickets: tickets }
    stutter
  }
...
";
    ...
    assert!(
        results.iter().any(|result| matches!(
            result,
            abide::verify::VerificationResult::Proved { name, method, .. }
                if name == "explicit_entity_choose_cross_call_return"
                    && method == "explicit-state exhaustive search"
        )),
        "entity choose cross-call returns should execute in explicit-state, got: {results:?}"
    );
}
