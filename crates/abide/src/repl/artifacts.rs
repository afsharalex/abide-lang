...
use crate::qa::artifacts::StateSpaceArtifact;
use abide_witness::{
    ...
    pub fn render_show(&self) -> String {
        ...
        match &self.payload {
            ArtifactPayload::Evidence(evidence) => match evidence.payload() {
                EvidencePayload::Witness(witness) => {
                    render_witness_summary(&mut out, witness);
                }
                ...
                _ => {
                    out.push_str("  evidence summary unavailable for this payload kind\n");
                }
            },
...
impl ArtifactStore {
    ...
    pub fn record_verify_results(&mut self, results: &[VerificationResult]) -> usize {
        ...
        for result in results {
            ...
            self.next_id += 1;
            self.artifacts.push(ReplArtifact {
                id: self.next_id,
                name,
                result_kind,
                payload,
    ...
    pub fn artifacts(&self) -> &[ReplArtifact] {
        &self.artifacts
    }

    pub fn artifact(&self, id: usize) -> Option<&ReplArtifact> {
        self.artifacts.iter().find(|artifact| artifact.id == id)
    }

    pub fn resolve(&self, selector: &str) -> Option<&ReplArtifact> {
...
mod tests {
    ...
    #[test]
    fn store_records_evidence_bearing_results() {
        ...
        assert_eq!(store.record_verify_results(&results), 1);
        assert_eq!(store.artifacts().len(), 1);
        assert!(store.artifact(1).is_some());
    }

    #[test]
    fn operational_artifact_renders_state_and_diff() {
        let artifact = ReplArtifact {
            id: 1,
            name: "safe".to_owned(),
            result_kind: "counterexample",
            payload: ArtifactPayload::Evidence(sample_operational_evidence()),
            evidence_extraction_error: None,
        };

        let state = artifact.render_state(1).expect("state view");
        assert!(state.contains("State 1"));
        assert!(state.contains("Order#0.status = OrderStatus::Shipped"));

        let diff = artifact.render_diff(0, 1).expect("diff view");
    ...
    #[test]
    fn simulation_artifact_renders_and_resolves() {
        let mut store = ArtifactStore::default();
        ...
        assert!(artifact.render_show().contains("simulation run"));
        assert!(artifact.render_state(0).expect("state").contains("State 0"));
    }
}
