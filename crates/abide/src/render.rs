//! Human-readable rendering of IR, diagnostics, witnesses, and
//! verification reports.
//!
//! Supports three output formats — JSON (for tooling), Markdown
//! (for reports), and HTML (for browser-rendered output) — via the
//! `VerifyReportFormat` enum. The bulk of this module is text
//! generation; the verifier's machine-readable outputs live elsewhere
...
pub(crate) struct VerificationReportInput<'a> {
    pub request: &'a VerifyReportRequest,
    pub files: &'a [PathBuf],
    pub config: VerificationReportConfig<'a>,
    pub diagnostics: &'a [Diagnostic],
    pub results: &'a [verify::VerificationResult],
}

pub(crate) struct VerificationReportConfig<'a> {
    pub solver: VerificationSolverConfig<'a>,
...
pub(crate) struct VerificationSolverConfig<'a> {
    pub solver_name: &'a str,
    pub chc_solver_name: &'a str,
}

pub(crate) struct VerificationModeConfig {
...
pub(crate) struct VerificationDisabledChecks {
    pub no_ic3: bool,
...
pub(crate) fn write_verification_report(
    input: VerificationReportInput<'_>,
) -> miette::Result<PathBuf> {
    let report_json = build_verification_report_json(&input);
    fs::create_dir_all(&input.request.output_dir)
        .into_diagnostic()
        .wrap_err_with(|| {
            format!(
                "failed to create report directory {}",
                input.request.output_dir.display()
            )
        })?;
    let report_path =
        derive_verify_report_path(&input.request.output_dir, input.files, input.request.format);
    let report_body = render_verification_report(
...
fn build_verification_report_json(input: &VerificationReportInput<'_>) -> serde_json::Value {
    ...
    serde_json::json!({
        ...
        "version": 1,
        "files": input.files.iter().map(|path| path.display().to_string()).collect::<Vec<_>>(),
        "summary": {
        ...
        "config": {
            "solver": config.solver.solver_name,
            ...
            "bmc_iterative_deepening": config.mode.bmc_iterative_deepening,
            "ic3_timeout_ms": config.timeouts.ic3_secs.saturating_mul(1000),
...
fn render_verification_report_html(
    ...
    let summary = HtmlReportSummary::from_results(results);
    let mut out = String::new();
    push_html_report_document_start(&mut out);
    push_html_report_header(&mut out, report_json, diagnostics, results, &summary);
    push_html_failure_results(&mut out, results, &summary.failure_indices);
    push_html_success_results(&mut out, results, &summary.success_indices);
    push_html_diagnostics(&mut out, diagnostics);
...
impl HtmlReportSummary {
    fn from_results(results: &[verify::VerificationResult]) -> Self {
        Self {
            failure_indices: results
                .iter()
                .enumerate()
                .filter_map(|(idx, result)| result.is_failure().then_some(idx))
                .collect(),
            success_indices: results
                .iter()
                .enumerate()
                .filter_map(|(idx, result)| (!result.is_failure()).then_some(idx))
                .collect(),
            proved_count: results
                .iter()
                .filter(|result| matches!(result, verify::VerificationResult::Proved { .. }))
                .count(),
            checked_count: results
                .iter()
                .filter(|result| matches!(result, verify::VerificationResult::Checked { .. }))
                .count(),
            admitted_count: results
                .iter()
...
fn push_html_report_document_start(out: &mut String) {
    out.push_str(
...
fn push_html_report_header(
    ...
    if summary.failure_indices.is_empty() {
    ...
    let mut summary_items = vec![
        ("Results", results.len()),
        ("Failures", summary.failure_indices.len()),
    ];
    if summary.proved_count > 0 {
        summary_items.push(("Proved", summary.proved_count));
    }
    if summary.checked_count > 0 {
        summary_items.push(("Checked", summary.checked_count));
    }
    if summary.admitted_count > 0 {
        summary_items.push(("Admitted", summary.admitted_count));
    }
    if !diagnostics.is_empty() {
...
fn push_html_diagnostics(out: &mut String, diagnostics: &[Diagnostic]) {
    if diagnostics.is_empty() {
        return;
    }
...
fn render_result_html_explanation(result: &verify::VerificationResult) -> Option<String> {
    match result {
        ...
        verify::VerificationResult::Deadlock {
            ...
            event_diagnostics,
            ..
        } => Some(render_deadlock_html_explanation(
            *step,
            reason,
            event_diagnostics,
        )),
        verify::VerificationResult::Counterexample { .. } => {
            Some(render_counterexample_html_explanation(result))
        }
        verify::VerificationResult::Unprovable { hint, .. } => {
            Some(render_unprovable_html_explanation(hint))
        }
        verify::VerificationResult::SceneFail { reason, .. } => Some(
            render_short_reason_html_explanation("Why this scene fails", reason),
        ),
        verify::VerificationResult::SceneUnknown { reason, .. } => Some(
            render_short_reason_html_explanation("Why this scene is unknown", reason),
        ),
        verify::VerificationResult::FnContractFailed { .. } => {
            Some(render_fn_contract_html_explanation(result))
        }
        _ => None,
    }
}

fn render_deadlock_html_explanation(
    step: usize,
    reason: &str,
    event_diagnostics: &[verify::DeadlockEventDiag],
) -> String {
    let mut out = String::new();
    ...
    out.push_str(&step.to_string());
    out.push_str(
        "</strong> with no enabled progress step, so the run cannot advance beyond this state.</p>",
    );
    out.push_str("<p><strong>Deadlock reason:</strong> ");
    out.push_str(&escape_html(reason));
    out.push_str("</p>");
    push_blocked_event_diagnostics_html(&mut out, event_diagnostics);
    out.push_str("</section>");
    out
}

fn push_blocked_event_diagnostics_html(
    out: &mut String,
    event_diagnostics: &[verify::DeadlockEventDiag],
) {
    if event_diagnostics.is_empty() {
...
fn render_counterexample_html_explanation(result: &verify::VerificationResult) -> String {
    let mut out = String::new();
    ...
    push_counterexample_evidence_explanation(&mut out, result);
    out.push_str("</section>");
    out
}

fn push_counterexample_evidence_explanation(out: &mut String, result: &verify::VerificationResult) {
    match result
        .evidence()
        .map(abide_witness::EvidenceEnvelope::payload)
    {
...
fn render_unprovable_html_explanation(hint: &str) -> String {
    let mut out = String::new();
...
fn render_short_reason_html_explanation(eyebrow: &str, reason: &str) -> String {
    let mut out = String::new();
    out.push_str("<section class=\"explanation-block\"><div class=\"trace-eyebrow\">");
...
fn render_fn_contract_html_explanation(result: &verify::VerificationResult) -> String {
    let mut out = String::new();
    ...
    out.push_str(
        "<p>The verifier found a violating function execution or proof obligation.</p></section>",
    );
    out
}
...
fn render_operational_trace_text(
    ...
    match trace {
        OperationalTraceView::Linear { steps } => render_linear_trace_text(&mut lines, steps, art),
        OperationalTraceView::Deadlock {
            steps,
            deadlock_step,
            reason,
            event_diagnostics,
        } => render_deadlock_trace_text(
            &mut lines,
            steps,
            *deadlock_step,
            reason,
...
fn render_linear_trace_text(lines: &mut Vec<String>, steps: &[RenderedTraceStep], art: TextArt) {
    ...
    lines.push(format!(
        ...
        art.elbow(),
        steps.len(),
        plural_suffix(steps.len())
    ));
    lines.extend(render_trace_section_lines(steps, 2, art));
}

fn render_deadlock_trace_text(
    lines: &mut Vec<String>,
    steps: &[RenderedTraceStep],
    deadlock_step: usize,
    reason: &str,
    event_diagnostics: &[RenderedEventDiagnostic],
    art: TextArt,
) {
    ...
    render_blocked_events_text(lines, event_diagnostics, art);
}

fn render_blocked_events_text(
    lines: &mut Vec<String>,
    event_diagnostics: &[RenderedEventDiagnostic],
    art: TextArt,
) {
...
struct LassoTraceTextInput<'a> {
    result: &'a verify::VerificationResult,
    prefix: &'a [RenderedTraceStep],
    loop_steps: &'a [RenderedTraceStep],
    loop_start: usize,
    fairness: &'a [RenderedFairnessEntry],
    art: TextArt,
}

fn render_lasso_trace_text(lines: &mut Vec<String>, input: LassoTraceTextInput<'_>) {
    let mut root_index = 0usize;
    let root_items = lasso_root_item_count(input.prefix, input.fairness);
    render_lasso_prefix_text(lines, &mut root_index, root_items, &input);
    render_lasso_cycle_summary_text(lines, &mut root_index, root_items, &input);
    render_lasso_fairness_text(lines, &mut root_index, root_items, &input);
}

fn lasso_root_item_count(
    prefix: &[RenderedTraceStep],
    fairness: &[RenderedFairnessEntry],
) -> usize {
    1 + usize::from(!prefix.is_empty()) + usize::from(!fairness.is_empty())
}

fn render_lasso_prefix_text(
    lines: &mut Vec<String>,
    root_index: &mut usize,
    root_items: usize,
    input: &LassoTraceTextInput<'_>,
) {
    ...
    *root_index += 1;
    lines.push(format!(
        "{} Lead-in",
        root_branch(*root_index, root_items, input.art)
    ));
    lines.extend(render_trace_section_lines(input.prefix, 2, input.art));
}

fn render_lasso_cycle_summary_text(
    lines: &mut Vec<String>,
    root_index: &mut usize,
    root_items: usize,
    input: &LassoTraceTextInput<'_>,
    ...
    *root_index += 1;
    ...
    render_lasso_cycle_facts_text(lines, input);
    lines.push(format!(
        "{} {}",
        indent_with_art(2, input.art),
        input.art.elbow()
    ));
    let last_index = lines.len() - 1;
    lines[last_index].push_str(" cycle steps");
    lines.extend(render_trace_section_lines(input.loop_steps, 4, input.art));
}

fn render_lasso_cycle_facts_text(lines: &mut Vec<String>, input: &LassoTraceTextInput<'_>) {
    let art = input.art;
    if let Some(cycle) = rendered_cycle_summary(input.loop_steps, input.loop_start) {
        lines.push(format!(
            "{} {} {}",
            indent_with_art(2, art),
            art.branch(),
    ...
    lines.push(format!(
        "{} {} {} step{}",
        indent_with_art(2, art),
        if input.loop_steps.is_empty() {
            art.elbow()
        } else {
            art.branch()
        },
        input.loop_steps.len(),
        plural_suffix(input.loop_steps.len())
    ));
    if matches!(
        input.result,
        verify::VerificationResult::LivenessViolation { .. }
    ...
    render_stable_loop_assignments_text(lines, input.loop_steps, art);
}

fn render_stable_loop_assignments_text(
    lines: &mut Vec<String>,
    loop_steps: &[RenderedTraceStep],
    art: TextArt,
) {
    let stable = stable_loop_assignments(loop_steps);
    if stable.is_empty() {
...
fn render_lasso_fairness_text(
    lines: &mut Vec<String>,
    root_index: &mut usize,
    root_items: usize,
    input: &LassoTraceTextInput<'_>,
) {
    if input.fairness.is_empty() {
        return;
    }
    *root_index += 1;
    lines.push(format!(
    ...
    for (idx, entry) in input.fairness.iter().enumerate() {
        let branch = if idx + 1 == input.fairness.len() {
            input.art.elbow()
        } else {
            input.art.branch()
        };
...
fn root_branch(index: usize, total: usize, art: TextArt) -> &'static str {
    if index == total {
        art.elbow()
    } else {
        art.branch()
...
fn render_operational_trace_html(result_idx: usize, trace: &OperationalTraceView) -> String {
    ...
    let (trace_heading, trace_summary) = trace_heading_summary(trace);
    ...
    if let Some(step) = single_step_operational_trace(trace) {
        push_single_step_trace_html(&mut out, step, trace, trace_heading, trace_summary);
        return out;
    }
    let initial_panel_id = initial_trace_panel_id(trace);
    push_trace_stage_header_html(&mut out, &trace_id, trace_heading, trace_summary, true);
    out.push_str("<div class=\"trace-frame\"><nav class=\"trace-nav\">");
    push_trace_nav_html(&mut out, &trace_id, trace);
    out.push_str("</nav><div class=\"trace-stage\">");
    push_trace_panels_html(&mut out, trace);
...
fn trace_heading_summary(trace: &OperationalTraceView) -> (&'static str, &'static str) {
    match trace {
        OperationalTraceView::Deadlock { .. } => (
            "Blocked state",
...
fn push_single_step_trace_html(
    out: &mut String,
    ...
    push_trace_stage_header_html(out, "", trace_heading, trace_summary, false);
    out.push_str("<div class=\"trace-stage\">");
    render_trace_panel_html(out, step, true, false);
    render_trace_footer_html(out, trace);
    out.push_str("</div></section>");
}

fn initial_trace_panel_id(trace: &OperationalTraceView) -> String {
    match trace {
...
fn push_trace_stage_header_html(
    out: &mut String,
    trace_id: &str,
    trace_heading: &str,
    trace_summary: &str,
    ...
    if include_controls {
        push_trace_controls_html(out, trace_id);
    }
    out.push_str("</div>");
}

fn push_trace_controls_html(out: &mut String, trace_id: &str) {
    out.push_str(
        "<div class=\"trace-controls\"><button type=\"button\" onclick=\"showAdjacentTraceStep('",
    );
    out.push_str(trace_id);
    out.push_str(
    ...
    out.push_str(trace_id);
    out.push_str("', 1)\">Next</button></div>");
}

fn push_trace_nav_html(out: &mut String, trace_id: &str, trace: &OperationalTraceView) {
    match trace {
        OperationalTraceView::Linear { steps } | OperationalTraceView::Deadlock { steps, .. } => {
            push_linear_trace_nav_html(out, trace_id, trace, steps);
        }
        OperationalTraceView::Lasso {
            prefix,
            loop_steps,
            ...
            push_lasso_trace_nav_html(out, trace_id, prefix, loop_steps, *loop_start);
        }
    }
}

fn push_linear_trace_nav_html(
    out: &mut String,
    trace_id: &str,
    trace: &OperationalTraceView,
    steps: &[RenderedTraceStep],
    ...
    for step in steps {
        ...
        let badge = match trace {
            OperationalTraceView::Deadlock { deadlock_step, .. } if step.step == *deadlock_step => {
                Some("blocked")
            }
            _ => None,
        };
        render_trace_step_button(out, trace_id, &panel_id, step, badge, false, false);
    }
}

fn push_lasso_trace_nav_html(
    out: &mut String,
    trace_id: &str,
    prefix: &[RenderedTraceStep],
    loop_steps: &[RenderedTraceStep],
    ...
    for step in prefix {
        let panel_id = format!("step-{}", step.step);
        render_trace_step_button(out, trace_id, &panel_id, step, None, false, false);
    }
    if !loop_steps.is_empty() {
...
fn push_trace_panels_html(out: &mut String, trace: &OperationalTraceView) {
    match trace {
        OperationalTraceView::Linear { steps } | OperationalTraceView::Deadlock { steps, .. } => {
            for (idx, step) in steps.iter().enumerate() {
                render_trace_panel_html(out, step, idx == 0, false);
            }
        }
        OperationalTraceView::Lasso {
            prefix,
